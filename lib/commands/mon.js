...
function mon(__, options, loader) {
  ...
  options.ocr = options.ocr || options.language || options.psm || options.ranges;
    ...
    if (options.parse) {
      ...
      receiver.on('data', function (data) {
        ...
        parser.parse(data).then(function (ast) {
          ...
          return textualize(ast.entries, options);
...
// textualize ast.entries
function textualize(entries, options) {
  options = _.assign({
    encoding: 'GB2312'
  }, options);
  return Promise.map(entries, function (entry) {
    switch (entry.type) {
      ...
      case 'raster':
      {
        if (options.ocr) {
          console.time('ocr');
          return ocr(entry.data, options).then(function (text) {
            console.timeEnd('ocr');
            return text;
          }).catch(function (err) {
            throw err;
