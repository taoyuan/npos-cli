...
mon.options = {
  ...
  tessdata: {
    alias: 't',
    describe: 'Specify tessdata path used for Ocr. Ocr enabled implied.'
  },
  language: {
    alias: 'l',
    describe: 'Specify language(s) used for Ocr. Ocr enabled implied.'
  },
  psm: {
    alias: 'm',
    describe: 'Specify page segmentation mode. Ocr enabled implied.'
  }
};
...
function translate(options) {
  options = options || {};
  options.ocr = !!(options.ocr || options.language || options.psm);
  return function (ctx, next) { // don't remove `next`
    ...
    if (options.ocr && ctx.image && (ctx.last || ctx.node.type === 'text')) {
      return require('npos-ocr').ocr(ctx.image, options).then(function (text) {
        ctx.text = ctx.text || '';
        ctx.text += text || '';
        ctx.image = null;
